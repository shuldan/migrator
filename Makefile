...
fmt: $(GOIMPORTS)
	...
	@$(GOIMPORTS) -local github.com/shuldan/migrator -w $$(find . -name "*.go" -not -path "./vendor/*")
...
fmt-check: $(GOIMPORTS)
	...
	@unformatted_imports=$$($(GOIMPORTS) -local github.com/shuldan/migrator -l . | grep -v vendor || true); \
